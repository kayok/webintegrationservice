...
{
    /// <summary>
    /// Example application that subscribes to web.SensorData.
    /// </summary>
    public static class SensorDataSubscriber
    {
        private static int ProcessData(DataReader<SensorData> reader)
        {
        ...
        public static void RunSubscriber(int domainId = 0, int sampleCount = int.MaxValue)
        {
            ...
            Topic<SensorData> topic = participant.CreateTopic<SensorData>("Example web_SensorData");

            // A Subscriber allows an application to create one or more DataReaders
            // Subscriber QoS is configured in USER_QOS_PROFILES.xml
            Subscriber subscriber = participant.CreateSubscriber();

            // This DataReader reads data on Topic "Example SensorData".
            // DataReader QoS is configured in USER_QOS_PROFILES.xml
            DataReader<SensorData> reader = subscriber.CreateDataReader(topic);
            ...
            while (samplesRead < sampleCount)
            {
                ...
                Console.WriteLine("SensorData subscriber sleeping for 4 sec...");
